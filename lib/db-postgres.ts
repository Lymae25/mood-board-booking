...
export async function getScenes(projectId: string) {
  try { const result = await sql`SELECT * FROM scenes WHERE projectId = ${projectId} ORDER BY sceneNumber ASC`; return result.rows } catch (e) { return [] }
}
...
export async function getSceneNotes(sceneId: string) {
  try { const result = await sql`SELECT * FROM sceneNotes WHERE sceneId = ${sceneId} ORDER BY createdAt DESC`; return result.rows } catch (e) { return [] }
}
...
export async function getIdeas(projectId: string) {
  try { const result = await sql`SELECT * FROM ideas WHERE projectId = ${projectId}`; return result.rows } catch (e) { return [] }
}
...
export async function getTimeline(projectId: string) {
  try { const result = await sql`SELECT * FROM timeline WHERE projectId = ${projectId}`; return result.rows } catch (e) { return [] }
}
