...
// ---------- Calendar feed token ----------
// GET /api/calendar/feed.ics used to be completely unauthenticated - anyone
// with the URL got every customer's name, every project, every meeting
// (with location) and every deadline. Apple/Google Calendar's ICS
// subscription mechanism can't send cookies, so instead of a session this
// uses a long random token embedded in the subscribe URL itself, the same
// scheme Google Calendar's own "private address" ICS links use.
export async function getOrCreateCalendarToken(): Promise<string | null> {
  const auth = await getAdminAuth()
  if (!auth) return null // admin not configured yet
  if (auth.calendarToken) return auth.calendarToken
  const sql = getDb()
  const token = randomBytes(24).toString('hex')
  await sql`UPDATE admin_auth SET "calendarToken" = ${token} WHERE "id" = 'singleton'`
  return token
}
...
export async function verifyCalendarToken(token: string | null | undefined): Promise<boolean> {
  if (!token) return false
  const auth = await getAdminAuth()
  if (!auth?.calendarToken) return false
  const a = Buffer.from(token)
  const b = Buffer.from(auth.calendarToken)
